...
export const validateEntryByPassV2 = functions.https.onCall(
 async (data: any, context: any) => {
   ...
   logger.info(`payloadJson length: ${payloadJson.length}`);
   logger.info(`payloadJson hex: ${Buffer.from(payloadJson, 'utf-8').toString('hex')}`);
   logger.info(`signatureB64Url: ${signatureB64Url}`);
   logger.info(`kid: ${kid}`);
   logger.info("====================================");
