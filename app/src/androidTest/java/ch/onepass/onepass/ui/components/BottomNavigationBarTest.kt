 ...
 *
 * The test scene exposes the current route through a small [Text] node with a testTag,
 * so assertions can be performed without a full NavHost graph.
 */
class BottomNavigationBarTest {
    ...
    /**
     * Test tag used to expose the current route as a plain [Text] node,
     * allowing stable assertions on navigation outcomes.
     */
    private val CURRENT_ROUTE_TAG = "current_route"

    /**
     * Sets up a minimal scene containing [BottomNavigationMenu] and a text field exposing the
     * current route through [CURRENT_ROUTE_TAG]. The [onNavigate] callback updates the local state,
     * mimicking how a NavController would change routes.
     *
    ...
    fun clickingEventsUpdatesRoute() {
        ...
        composeTestRule.onNodeWithTag(CURRENT_ROUTE_TAG)
    ...
    /**
     * Confirms that the functional order of tabs matches [NavigationDestinations.tabs].
     * For each label in order, we click and verify that the exposed route matches the expected destination.
     */
    @Test
    ...
    /**
     * Ensures the bottom bar remains usable when provided an unknown initial route:
     * tabs remain visible and clicking an item sets a valid route.
     */
    @Test
