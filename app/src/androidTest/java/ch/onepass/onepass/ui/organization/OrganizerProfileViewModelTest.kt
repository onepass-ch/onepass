...
class OrganizerProfileViewModelTest {
  ...
  private class MockOrganizationRepository(
    ...
    override fun getPendingInvitations(
        organizationId: String
    ): Flow<List<OrganizationInvitation>> =
        flowOf(emptyList())
  ...
  fun loadOrganizationProfile_eventLoadingError_emitsErrorEffect() = runTest {
      ...
      Assert.assertTrue(
          "Should emit error effect",
          effects.any { it is OrganizerProfileEffect.ShowError })
  ...
  fun onFollowClicked_alreadyFollowing_togglesToNotFollowing() = runTest {
      ...
      testDispatcher.scheduler.advanceUntilIdle()
      Assert.assertFalse(
          "Should be unfollowing after second click",
          viewModel.state.value.isFollowing
      )
  ...
  fun onWebsiteClicked_withUrl_emitsOpenWebsiteEffect() = runTest {
      ...
      Assert.assertTrue(
          "Should emit OpenWebsite effect",
