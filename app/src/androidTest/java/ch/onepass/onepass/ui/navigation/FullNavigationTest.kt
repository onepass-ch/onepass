...
class OnePassAppRoutesTest {
    ...
    // 1) Auth → Events
    @Test fun auth_navigates_to_events() {
        setApp()
        tapTestLogin()
    ...
    @Test fun bottomBar_navigates_to_tickets() {
        setApp()
        tapTestLogin()
    ...
    @Test fun bottomBar_navigates_to_profile() {
        setApp()
        tapTestLogin()
    ...
    @Test fun profile_sign_out_returns_to_auth() {
        setApp()
        tapTestLogin()
        compose.onNodeWithTag("BOTTOM_TAB_PROFILE").assertIsDisplayed().performClick()
