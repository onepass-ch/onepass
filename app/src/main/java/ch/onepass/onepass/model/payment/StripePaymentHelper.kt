 ...
 *
 * This class provides methods to handle payment flows using Stripe's Payment Sheet. The
 * PaymentSheet instance should be provided from MainActivity via CompositionLocal to avoid
 ...
 * ```
 */
class StripePaymentHelper(private val paymentSheet: PaymentSheet?) {
  ...
  /**
   * Secondary constructor for backwards compatibility. Creates the helper without a PaymentSheet
   * (will fail when trying to present).
   */
  constructor() : this(null)

  /**
   * Constructor that initializes with an activity (for backwards compatibility). Note: This may
   * cause lifecycle issues if called during composition. Prefer using the constructor that takes a
   * PaymentSheet instance.
   */
  constructor(
  ...
  init {
    ...
    if (paymentSheet != null) {
      PaymentSheetResultHandler.setHandler { result -> handlePaymentSheetResult(result) }
    }
  }
  ...
  /**
   * Creates a customer configuration for returning customers. This allows users to see and select
