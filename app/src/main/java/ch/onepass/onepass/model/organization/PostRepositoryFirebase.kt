 ...
 *
 * This repository handles all post-related CRUD operations using Firebase Firestore. Posts are
 * stored in a top-level "posts" collection and queried by organization ID.
 */
class PostRepositoryFirebase : PostRepository {
  ...
  override fun getPostsByOrganization(organizationId: String): Flow<List<Post>> =
      firestoreFlow<Post> { postsCollection.whereEqualTo("organizationId", organizationId) }
          .map { posts ->
