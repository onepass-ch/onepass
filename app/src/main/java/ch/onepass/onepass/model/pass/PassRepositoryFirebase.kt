 ...
 *
 * SECURITY NOTE:
 * lastScannedAt is updated server-side by the Cloud Function "validateEntry" after ticket verification,
 * not by the client (prevents fraud).
 */
class PassRepositoryFirebase(
    ...
    /**
     * Returns the existing Pass or generates a new one (server-signed) if missing.
     * The Cloud Function writes to Firestore and RETURNS the pass directly; no polling needed.
     */
    override suspend fun getOrCreateSignedPass(uid: String): Result<Pass> = runCatching {
        require(uid.isNotBlank()) { "uid required" }

        // 1) If present in Firestore, return it
        users.document(uid).get().await().toPass()?.let { return@runCatching it }
    ...
    /**
     * Maps the Cloud Function response (expected to be a Map) to a Pass.
     * Required: uid, kid, issuedAt (secs), version, active, signature.
     * Optional: lastScannedAt, revokedAt.
     */
    private fun dataToPassStrict(data: Any?): Pass {
        val m: Map<String, Any?> =
            (data as? Map<*, *>)?.entries
                ...
                ?.toMap()
                ?: error("Invalid function response: expected a map")

        fun anyToSeconds(value: Any?): Long? = when (value) {
    ...
    /**
     * Maps a user document to a Pass by reading the nested "pass" field.
     * Returns null if the pass is absent or incomplete.
     * NOTE: uid MUST be present in pass; otherwise we throw to surface the data issue.
     */
