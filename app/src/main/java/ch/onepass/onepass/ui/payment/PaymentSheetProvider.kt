...
/**
 * CompositionLocal to provide PaymentSheet instance throughout the app.
 * This ensures PaymentSheet is initialized in MainActivity.onCreate before
 * the Activity reaches RESUMED state, avoiding lifecycle registration issues.
 */
val LocalPaymentSheet = compositionLocalOf<PaymentSheet?> { null }

/**
 * Creates a PaymentSheet instance for the given activity.
 * This should be called in MainActivity.onCreate to ensure proper lifecycle timing.
 * 
...
/**
 * Global handler for PaymentSheet results.
 * This is needed because PaymentSheet's result callback is set in the constructor,
 * but we want to handle results in the composable where StripePaymentHelper is used.
 */
