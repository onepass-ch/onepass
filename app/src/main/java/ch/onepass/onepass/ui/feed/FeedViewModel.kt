...
open class FeedViewModel(
  ...
  private fun applyFiltersLocally(
    ...
    return events.filter { event ->
      ...
      // Note: We already filter out sold out events in filterActiveEvents()
      // This filter is for when users explicitly want to hide sold out events via UI filter
  ...
  /** Refreshes the events list manually with updated recommendations. */
